<div align="center">
  ...
  <p>
    <a href="CHANGELOG.md"><img alt="Version"   src="https://img.shields.io/badge/version-1.5.0-blue"></a>
...
Every driver the panel has seen gets a shareable URL at `/p/<steam-id>` — no login required. The page is server-rendered HTML with the panel's own design tokens (light/dark theme, accent colour, typography) and shows: hero card with avatar, nickname, in-game name, country flag and a clickable Steam-chip linking to the public Steam profile; four KPI cards (total laps, time on track, best lap, server records held); a **Server records** table listing every `(track, layout, car)` combo where the driver holds the panel-wide MIN — track and layout names resolved through `ui_track.json` so they match the **Sesión activa** card down to the variant ("Grand Prix" instead of `layout_gp`); a **Personal bests** table across every combo the driver has driven. OpenGraph + Twitter card meta tags ship name + headline stats so a link pasted in Discord previews properly. A companion `GET /api/public/players/<steam-id>` returns the same shape as JSON for Discord bots / Twitch overlays. Both endpoints rate-limit at 120/min/IP and respect the **Configuración → Perfiles públicos** admin toggle (default on; flipping it off turns both URLs into a generic 404 server-wide). A small share button on each row of the **Players → Connection history** table copies the public URL to clipboard with one click.
