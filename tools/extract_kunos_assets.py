...
try:
    ...
    print("Error: The 'Pillow' library is required to convert images to WebP.")
...
def copy_image_webp(src, dest):
    """Converts an image to WebP format and saves it."""
    if os.path.exists(src):
        os.makedirs(os.path.dirname(dest), exist_ok=True)
        try:
            with Image.open(src) as img:
                # Convert to RGB if it's RGBA and we are saving without alpha,
                # but WebP supports alpha, so we can save directly.
                img.save(dest, 'webp', quality=85)
            return True
        except Exception as e:
            print(f"Failed to convert {src} to WebP: {e}")
            return False
    return False
...
def extract_cars(cars_src, dest_base):
    ...
    for car_id in os.listdir(cars_src):
        ...
        if os.path.exists(os.path.join(ui_path, "ui_car.json")):
            ...
            copy_image_webp(os.path.join(ui_path, "badge.png"), os.path.join(dest_car_ui, "badge.webp"))
                ...
                if os.path.exists(os.path.join(skin_path, "ui_skin.json")):
                    ...
                    for preview in previews:
                        ...
                        if ext in ['jpg', 'jpeg', 'png']:
                            copy_image_webp(preview, os.path.join(dest_skin, "preview.webp"))
...
def extract_tracks(tracks_src, dest_base):
    ...
    for track_id in os.listdir(tracks_src):
        ...
        if os.path.exists(os.path.join(ui_path, "ui_track.json")):
            ...
            copy_image_webp(os.path.join(ui_path, "preview.png"), os.path.join(dest_track_ui, "preview.webp"))
        ...
        for layout_id in os.listdir(ui_path):
            ...
            if os.path.exists(os.path.join(layout_path, "ui_track.json")):
                ...
                copy_image_webp(os.path.join(layout_path, "preview.png"), os.path.join(dest_layout, "preview.webp"))
